...
### Medication
```{r}
(fit_Med <- survfit(Surv(Begin,End, Kinetic) ~ factor(Medication), data = fog))
coxph_Med <- coxph(Surv(Begin,End, Kinetic) ~ factor(Medication), data = fog)
...
### Task type 
```{r}
(fit_task <- survfit(Surv(Begin,End, Kinetic) ~ factor(Task), data = fog))
coxph_task <- coxph(Surv(Begin,End, Kinetic) ~ factor(Task), data = fog)
...
### events Type 
```{r }
(fit_type <- survfit(Surv(Begin,End, Kinetic) ~ factor(Type), data = fog))
coxph_type <- coxph(Surv(Begin,End, Kinetic) ~ factor(Type), data = fog)
...
coxph_visit <- coxph(Surv(Begin,End, Kinetic) ~ factor(Visit), data = fog)
...
tidy(fit_mult) %>% 
  filter(p.value <= 0.05)
```
Medication and Visit present a p value < 0.05. Medication increases the risk of experiencing FOG by 0.7 holding visit fixed, while , visit does the same with a by a rate of 0.2 while holding Medication fixed. 
