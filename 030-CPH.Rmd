...
## Survival Curves
```{r}
sfit <- survfit(Surv(time, status) ~ sex, data = dat)
plot(sfit, col = 1:2)
```
...
data_new <- data.frame(sex = factor(c("f", "m")))
pred.cph <- survfit(fit.cph, newdata = data_new, type = "aalen")
